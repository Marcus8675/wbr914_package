...
void CmdVelListener::set_dio_service(const std::shared_ptr<wbr914_package::srv::DioSet::Request> request,
          std::shared_ptr<wbr914_package::srv::DioSet::Response> response)
          {

            if (wbr914.SetDio(request->digitalout)==0)
            {
              response->success = true;
            }
            else
            {
              RCLCPP_ERROR(this->get_logger(), "Couldn't set digital output on robot using SetDio");
              response->success = false;
              return;
            }
          }

void CmdVelListener::get_dio_service(const std::shared_ptr<wbr914_package::srv::DioGet::Request> request,
          std::shared_ptr<wbr914_package::srv::DioGet::Response> response)
          {
            uint16_t data;

            if (wbr914.GetDio(&data)>0)
            {
              response->success = true;
              response->digitalin = data;
            }
            else
            {
              RCLCPP_ERROR(this->get_logger(), "Couldn't read digital inputs on robot using GetDigitalData");
              response->success = false;
              return;
            }
          }

void CmdVelListener::timer_callback()
  {
    float voltages[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    float ranges[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    float scale = 128/2.8;
    ...
    odineye.set_led(3,128,0,128,0);  //heartbeat on
    if (wbr914.GetIRData(voltages, ranges)==0)
    {
      odineye.set_led(0,0,128,0,0); //Overall status
      //LED4
      if (voltages[4]<0.25)  //less then .25V is >80cm
      {
        odineye.set_led(4,0,128,0,0);  //IR5 Status 
      }
      else
      {
        if (voltages[4]>2.8) voltages[4]=2.8;
        howclose = voltages[4]*scale;
        odineye.set_led(4,howclose,(128-howclose),0,0);  //IR5 Status  highest value is 2.8V @10cm range
      }
      //LED5
      if (voltages[3]<0.25)  //less then .25V is >80cm
      {
        odineye.set_led(5,0,128,0,0);  //IR4 Status 
      }
      else
      {
        if (voltages[3]>2.8) voltages[3]=2.8;
        howclose = voltages[3]*scale;
        odineye.set_led(5,howclose,(128-howclose),0,0);  //IR4 Status  highest value is 2.8V @10cm range
      }
      //LED6
      if (voltages[2]<0.25)  //less then .25V is >80cm
      {
        odineye.set_led(6,0,128,0,0);  //IR3 Status 
      }
      else
      {
        if (voltages[2]>2.8) voltages[2]=2.8;
        howclose = voltages[2]*scale;
        odineye.set_led(6,howclose,(128-howclose),0,0);  //IR3 Status  highest value is 2.8V @10cm range
      }
      //LED1
      if (voltages[1]<0.25)  //less then .25V is >80cm
      {
        odineye.set_led(1,0,128,0,0);  //IR2 Status 
      }
      else
      {
        if (voltages[1]>2.8) voltages[1]=2.8;
        howclose = voltages[1]*scale;
        odineye.set_led(1,howclose,(128-howclose),0,0);  //IR2 Status  highest value is 2.8V @10cm range
      }
      //LED2
      if (voltages[0]<0.25)  //less then .25V is >80cm
      {
        odineye.set_led(2,0,128,0,0);  //IR1 Status 
      }
      else
      {
        if (voltages[0]>2.8) voltages[0]=2.8;
        howclose = voltages[0]*scale;
        odineye.set_led(2,howclose,(128-howclose),0,0);  //IR1 Status  highest value is 2.8V @10cm range
      }
    }
    else
    {
      odineye.set_led(0,128,0,0,0); //Overall status
    }
    odineye.set_led(3,0,0,0,0);  //heartbeat off
